...
1. Explain how to build stateful class components.
  ...
  We start building out a class component that extends the React.Component parent class. Use the constructor function to set up some state, because we’re calling 'extends', we also need to call 'super();' otherwise we won’t have access to the 'this' keyword. Then we need to render some sort of UI to the DOM, we do this by calling the life-cycle method render.

2. Describe the different phases of the component lifecycle.
  
  There are 3 phases to the React Component Lifecycle. First, is the "birth" or mounting phase, in this phase when the component is being built out from the ground up. The render method is invoked, and componentDidMount gets called as well. Second, is the "growth" or updating phase, here you are just updating component data. During this phase, setState can be used to change the component’s state data, forcing a call to render and also shouldComponentUpdate is a method one could use here to stop a component from calling render if necessary. The final phase is the "death" or un-mounting phase, pretty self-explanatory but the un-mounting phase includes removing the component from the screen. Here the components removed from the screen and also 'componentWillUnmount' is called and can be used to clean up. 

3. Demonstrate an understanding of class component lifecycle methods.

  Demonstrated in the above answer.
